...
const mainNavItems: NavItem[] = [
  { title: 'Dashboard', url: '/policies', icon: LayoutDashboard },
  { title: 'Central de Políticas', url: '/policies/central', icon: FileText },
  { title: 'Fluxos de Aprovação', url: '/policies/workflows', icon: GitBranch },
  { title: 'Campanhas de Aceite', url: '/policies/aceite', icon: Users },
  { title: 'Biblioteca de Modelos', url: '/policies/templates', icon: BookTemplate },
];

const configNavItems: NavItem[] = [
  { title: 'Configurações', url: '/policies/configuracoes', icon: Settings },
];
  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-sidebar-border">
      <SidebarHeader className="p-4">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              ...
              {organizations.length > 1 ? (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-64">
                    ...
                    {organizations.map((org) => (
                      <DropdownMenuItem key={org.id} onClick={() => handleSwitchOrganization(org.id)} className="flex items-center justify-between">
                ...
                <span className="text-xs text-sidebar-foreground/60 truncate">
                  {organization?.name || 'Gestão de Políticas'}
                </span>
              )}
            </div>
          )}
        ...
        {!collapsed && (
          <div className="mt-3">
            <div className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              <FileText className="w-4 h-4" />
              <span className="font-medium truncate flex-1 text-left">Gestão de Políticas</span>
            </div>
          </div>
        )}
      </SidebarHeader>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              ...
              <SidebarMenuItem>
                <SidebarMenuButton onClick={() => navigate('/vrm')} tooltip="Fornecedores (VRM)" className="text-purple-500 hover:text-purple-600 hover:bg-purple-500/10">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent side="top" align="start" className="w-56">
                <DropdownMenuItem asChild>
                  <RouterNavLink to="/policies/configuracoes" className="cursor-pointer">
